...
const updateCurrentUserInputSchema = z.object({
  units: z.enum(["mm", "in", "inches"]), // allowing 'inches' just in case, but we'll normalize to 'in' or 'mm' usually. Let's stick to 'mm' and 'in' as per schema comment, but user prompt said "mm or inches". I'll map "inches" to "in" if needed or just use "in" as the value.
});

export const updateCurrentUser = async (
  args: z.infer<typeof updateCurrentUserInputSchema>,
  context: any
) => {
  if (!context.user) {
